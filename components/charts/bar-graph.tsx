...
export function BarGraph() {
  ...
  const total = React.useMemo(
    () => ({
      avg_earnings: chartData?.data?.reduce(
        (acc, curr) => acc + curr.avg_earnings,
        0
      ),
      user_count: chartData?.data?.reduce(
  ...
  const chartTitleConfig = {
    avg_earnings: {
      ...
      description: 'Ventes et revenus moyens par jour'
    },
    user_count: {
      title: 'Total Clients',
    ...
    <Card className="h-full">
      <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
          <CardTitle>{chartTitleConfig[activeChart]?.title}</CardTitle>
          <CardDescription>
            {chartTitleConfig[activeChart]?.description}
          </CardDescription>
        </div>
        <div className="flex ">
          {['avg_earnings', 'user_count'].map((key) => {
      ...
      <CardContent className="px-2 sm:p-6 ">
        ...
        >
          ...
          >
            <CartesianGrid className="text-bgcolor1" vertical={false} />
            <XAxis
              dataKey="date"
              tickLine={false}
              axisLine={false}
